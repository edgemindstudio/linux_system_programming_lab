...
Complete program: [`experiments/chapter02_fd_reuse.c`](../experiments/chapter02_fd_reuse.c)

Run it:

```bash
./build/experiments/chapter02_fd_reuse
```
...
```bash
./build/experiments/chapter02_fd_reuse > redirected.txt
cat redirected.txt
```
...
Complete program: [`experiments/chapter02_open_flags.c`](../experiments/chapter02_open_flags.c)

```bash
./build/experiments/chapter02_open_flags
```
...
```bash
strace -e trace=openat,read,write,lseek,close,unlink ./build/experiments/chapter02_open_flags
```
...
Complete program: [`experiments/chapter02_permissions_umask.c`](../experiments/chapter02_permissions_umask.c)

```bash
./build/experiments/chapter02_permissions_umask
stat -c '%A %a %n' build/chapter02/data/permissions_demo.txt
```
...
Complete program: [`exercises/chapter02_08_robust_copy.c`](../exercises/chapter02_08_robust_copy.c)

Create input, copy it, and verify byte-for-byte equality:

```bash
printf 'alpha\nbeta\n' > build/chapter02/data/source.txt
./build/exercises/chapter02_08_robust_copy build/chapter02/data/source.txt build/chapter02/data/copied.txt
cmp build/chapter02/data/source.txt build/chapter02/data/copied.txt
...
```bash
./build/exercises/chapter02_08_robust_copy /bin/ls build/chapter02/data/copied_ls
cmp /bin/ls build/chapter02/data/copied_ls
file build/chapter02/data/copied_ls
```
...
Complete program: [`experiments/chapter02_nonblocking_pipe.c`](../experiments/chapter02_nonblocking_pipe.c)

```bash
./build/experiments/chapter02_nonblocking_pipe
```
...
Complete program: [`experiments/chapter02_durable_write.c`](../experiments/chapter02_durable_write.c)

```bash
./build/experiments/chapter02_durable_write
cat build/chapter02/data/durable_record.txt
```
...
Complete program: [`experiments/chapter02_direct_io.c`](../experiments/chapter02_direct_io.c)

```bash
./build/experiments/chapter02_direct_io
stat -c '%s bytes' build/chapter02/data/direct_io_demo.bin
```
...
Complete program: [`experiments/chapter02_sparse_file.c`](../experiments/chapter02_sparse_file.c)

```bash
./build/experiments/chapter02_sparse_file
ls -lh build/chapter02/data/sparse_demo.bin
du -h build/chapter02/data/sparse_demo.bin
```
...
```bash
cp --sparse=never build/chapter02/data/sparse_demo.bin build/chapter02/data/full_demo.bin
ls -ls build/chapter02/data/sparse_demo.bin build/chapter02/data/full_demo.bin
```
...
Complete program: [`exercises/chapter02_10_positional_io.c`](../exercises/chapter02_10_positional_io.c)

```bash
./build/exercises/chapter02_10_positional_io
```
...
Complete program: [`exercises/chapter02_11_truncate_file.c`](../exercises/chapter02_11_truncate_file.c)

```bash
./build/exercises/chapter02_11_truncate_file
```
...
Complete program: [`exercises/chapter02_12_select_stdin.c`](../exercises/chapter02_12_select_stdin.c)

Deterministic piped-input test:

```bash
printf 'ready\n' | ./build/exercises/chapter02_12_select_stdin
```
...
1. **Descriptor redirection.** Run `chapter02_fd_reuse` normally, then redirect standard
...
3. **Safe exclusive creation.** Modify `experiments/chapter02_open_flags.c` so it does not unlink its old
...
5. **Tiny-buffer copy.** Change `BUFFER_SIZE` in `exercises/chapter02_08_robust_copy.c` to `1`, rebuild, and
...
6. **Count system calls.** Compare the 1-byte and 4096-byte copy versions using:
   ...
   ```bash
   strace -c ./build/exercises/chapter02_08_robust_copy /bin/ls build/chapter02/data/copied_ls
   ```
...
8. **Nonblocking retry without spinning.** Extend `experiments/chapter02_nonblocking_pipe.c` so it uses
...
11. **Misaligned direct I/O.** In `experiments/chapter02_direct_io.c`, deliberately use an ordinary `malloc()`
