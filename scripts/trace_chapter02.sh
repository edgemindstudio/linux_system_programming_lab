...
if [[ $# -lt 1 ]]; then
    ...
    printf 'example: %s chapter02_06_read_file /etc/hosts\n' "$0" >&2
    exit 64
fi
...
for candidate in \
    "$repository_root/build/exercises/$program_name" \
    "$repository_root/build/experiments/$program_name"
do
    if [[ -x $candidate ]]; then
...
if [[ -z $executable ]]; then
    ...
    printf 'Run: make chapter02\n' >&2
    exit 66
fi

output_directory="$repository_root/observations/strace/chapter02"
